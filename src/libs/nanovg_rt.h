...
struct RTNVGtexture {
  int id;
  unsigned int tex;
  std::bitset<sizeof(unsigned int)> btk_reserved;
  int width, height;
  int type;
  int flags;
  unsigned char *data;
...
void nvgResizeFrameBufferRT(NVGcontext *ctxt,int w,int h){
    ...
    rt->width = w;
    rt->height = h;
    rt->pixels = (unsigned char*)realloc(rt->pixels,rt->width * rt->height * 4);
}
...
//   return tex->tex;
// }
void nvgFrameBufferSizeRT(NVGcontext *ctx,int *w,int *h){
    RTNVGcontext *rt = (RTNVGcontext *)nvgInternalParams(ctx)->userPtr;
    if(w != nullptr){
      *w = rt->width;
    }
...
void nvgClearBackgroundRT(NVGcontext *ctx,uint8_t r,uint8_t g,uint8_t b,uint8_t a) {
  RTNVGcontext *rt = (RTNVGcontext *)nvgInternalParams(ctx)->userPtr;
  for (size_t i = 0; i < rt->width * rt->height; i++) {
    rt->pixels[4 * i + 0] = r;
    rt->pixels[4 * i + 1] = g;
